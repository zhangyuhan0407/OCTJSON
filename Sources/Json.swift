...
//    public static func deserialize<ByteSequence: Collection , ByteSequence.Iterator.Element == UInt8>(_ sequence: ByteSequence) throws -> Json {
...
extension Json {
    public enum SerializationStyle {
        ...
        var serializer: JsonSerializer.Type {
            switch self {
            case .default:
...
extension Json {
    public subscript(index: Int) -> Json {
        assert(index >= 0)
        guard let array = self.array , index < array.count else { return JSON.null }
        return array[index]
    }
